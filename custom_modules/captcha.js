...
class Captcha {
  ...
  image() {
    return (req, res, next) => {
      ...
      for (var i = 0; i < 2; i++) {
        ctx.moveTo(
          Math.floor(0.08 * this.params.canvasWidth),
      ...
      // draw text:
      const text = ("" + crypto.randomBytes(4).readUIntBE(0, 4)).substr(
        2,
        this.params.codeLength
      );
      text.split("").forEach((char, i) => {
        ctx.setTransform(
          ...
          Math.random() * 0.4,
          Math.random() * 0.5 + 1,
          Math.floor(0.375 * this.params.fontSize) * i +
            Math.floor(0.25 * this.params.fontSize),
...
export const create = (params) => new Captcha(params);
